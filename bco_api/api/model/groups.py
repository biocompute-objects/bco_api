...
def post_api_groups_create(request):
    ...
    for creation_object in bulk_request:
        ...
        if standardized not in groups:
            ...
            group_admin.groups.add(Group.objects.get(name=creation_object["name"]))

            if (
                "expiration" not in creation_object
                or creation_object["expiration"] == ""
            ):
            ...
            if len(users_excluded) > 0:
                ...
                return_data.append(
                    db_utils.messages(parameters={"group": standardized})[
                        "201_group_create"
                    ]
                )
        ...
        if standardized in groups:
            ...
            if requestor_info.username == group_information.owner_user.username:
                ...
                if deleted_count < 2:
                    ...
                    return_data.append(
                        db_utils.messages(parameters={"group": grouped.name})[
...
def post_api_groups_modify(request):
    ...
    for modification_object in bulk_request:
        ...
        if standardized in groups:
            grouped = Group.objects.get(name=standardized)
            if (
                requestor_info.is_superuser == True
                or grouped in requestor_info.groups.all()
            ):
