...
def POST_api_objects_drafts_token(rqst, internal=False):
    ...
    if not internal:
        print(
            " Not Internal, user response: {}".format(
                user_objects.intersection(prefix_objects).values(*return_values)
            )
        )
        ...
        for pub in published:
            ...
            try:
                bco_url, bco_id_accession, bco_id_version = pub["object_id"].rsplit(
                    "/", 2
                )
                bco_id_name = bco_url + "/" + bco_id_accession
            except Exception as error:
                print(
                    "Biocompute Name, Version, and URL not formatted as expected: {}".format(
                        error
                    )
                )
        ...
        result_list = chain(
            unique_published.values(*return_values),
