...
def get_schema(schema_uri):
    ...
    try:
        ...
        return {
            schema_uri: ['Failed to load extension schema. JSON Decode Error.']
        }

    except TypeError:
        return {
            schema_uri: ['Failed to load extension schema. Invalid format.']
        }

    except ErrorConnecting:
        return {
            schema_uri: ['Failed to load extension schema. Connection Error.']
        }
...
def post_validate_bco(request):
    ...
    try:
        ...
        file_path = os.path.dirname(os.path.abspath(\
            'api/validation_definitions/IEEE/2791object.json')
        )

        ieee = 'api/validation_definitions/IEEE/2791object.json'
        with open(ieee, 'r', encoding='utf-8') as file:
            schema = jsonref.load(file, base_uri=f'file://{file_path}/', jsonschema=True)
    ...
    for bco in bco_list:
        ...
        if 'extension_domain' in bco.keys():
            for extension in bco['extension_domain']:
                ...
                results[identifier]['number_of_errors'] += \
                    results[identifier][extension_id]['number_of_errors']
