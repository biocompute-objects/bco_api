...
class BcoTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.object_id = "http://localhost:8000/{}{}".format(
            self.object_id_root, self.object_id_version
        )
    ...
    def test_bco_creation(self):
        ...
        self.assertEqual(
            biocompute.owner_group, Group.objects.get(name=self.owner_group)
        )
        self.assertEqual(
            biocompute.owner_user, User.objects.get(username=self.owner_user)
        )
