...
class BCOViewTestCase(TestCase):
    ...
    def test_view_published_bco_unauthorized(self):
        ...
        response = self.client.get('/api/BCO_000001/')
        self.assertEqual(response.status_code, 401)

    def test_view_published_bco_forbidden(self):
        # Request with valid object ID but unauthorized user
        # (e.g., user without sufficient permissions to access the BCO)
        object_id = "BCO_000001"
        # Authenticate as a user without sufficient permissions (if applicable)
